...
int main(void)
{
	...
	int* idx = (int*)malloc(NUM_POINTS * sizeof(int));//correspondence for P points
	float start, end;
	start = dsecnd();
	for (int i = 0; i < NUM_POINTS; i++)//for each p point
	{
		max_dist = 1000000000.0f;//initialize for each p point
		for (int j = 0; j < NUM_POINTS; j++)//for each q point
		{
	...
	fopen_s(&document, "Correspondence.csv", "w");//open the file
	if (!document) {
		perror("File opening failed");
		return (-1);
	}
